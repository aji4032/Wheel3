 ...
 * such as Browser, DOM, Input, Log, Network, Overlay, Page, Performance,
...
public class CdpUtility {
     ...
     * @param text                  Text as generated by processing a virtual key
     *                              code with a keyboard layout.
     * @param keyIdentifier         Unique identifier of the key.
     * @param code                  Unique DOM defined string value for each
     *                              physical key (e.g., 'KeyA').
     * @param key                   Unique DOM defined string value describing the
     *                              meaning of the key in the context of active
     *                              modifiers, keyboard layout, etc (e.g., 'Alt',
     *                              'F1', 'a').
     * @param windowsVirtualKeyCode Windows virtual key code.
     * @param nativeVirtualKeyCode  Native virtual key code.
     * @return The command result.
     */
    public JsonNode inputDispatchKeyEvent(String type, int modifiers, String text, String keyIdentifier, String code,
            String key, int windowsVirtualKeyCode, int nativeVirtualKeyCode) {
        Map<String, Object> map = new HashMap<>();
        map.put("type", type);
     ...
     * @param x          X coordinate of the event relative to the main frame's
     *                   viewport in CSS pixels.
     * @param y          Y coordinate of the event relative to the main frame's
     *                   viewport in CSS pixels.
     * @param modifiers  Bit field representing pressed modifier keys.
     ...
     * @return The command result.
     */
    public JsonNode inputDispatchMouseEvent(MouseEvent eventType, int x, int y, int modifiers, String button,
            int clickCount) {
        Map<String, Object> map = new HashMap<>();
     ...
     * @param returnByValue Whether the result is expected to be a JSON object that
    ...
    /**
     * Creates a new isolated browser context (similar to incognito).
     * Pages created in this context have their own cookies, localStorage, etc.
     *
     * @return The browserContextId string.
     */
    public String targetCreateBrowserContext() {
        JsonNode result = executeCdpCommand("Target.createBrowserContext", Map.of(), defaultDuration);
        return result.get("browserContextId").asText();
    }

    /**
     * Disposes a browser context previously created via
     * {@link #targetCreateBrowserContext()}.
     * All pages belonging to this context are closed.
     *
     * @param browserContextId The context to dispose.
     */
    public void targetDisposeBrowserContext(String browserContextId) {
        Map<String, Object> map = new HashMap<>();
        map.put("browserContextId", browserContextId);
        executeCdpCommand("Target.disposeBrowserContext", map, defaultDuration);
    }

    /**
     * Creates a new page target within a given browser context.
     *
     * @param url              The initial URL of the page (use "about:blank" for
     *                         empty).
     * @param browserContextId The browser context to create the page in.
     * @return The targetId of the newly created page.
     */
    public String targetCreateTarget(String url, String browserContextId) {
        Map<String, Object> map = new HashMap<>();
        map.put("url", url);
        map.put("browserContextId", browserContextId);
